...
# ══════════════════════════════════════════════════════════════════════════
# GRAPHIC ANALYSIS 2 — Geo-Alignment: Streaming Countries vs. Tour Countries
# ══════════════════════════════════════════════════════════════════════════

st.divider()
st.markdown("""
<div class="rq-box">
    <h3>🌍 Graphic Analysis 2</h3>
    ...
    Ranked by <em>listeners_in_country</em> — unique listeners per country.</p>
</div>
""", unsafe_allow_html=True)

st.markdown("""
**Idee:** Wenn ein Artist in Deutschland sehr beliebt auf Last.fm ist, tourt er dann auch
tatsaechlich in Deutschland? Oder gibt es Artists die digital in einem Land gross sind,
...
with c1:
    ...
    <div class="insight-card">
        ...
        <p>Ueberlappung der Top-10 Streaming-Laender (nach Listeners)
        mit den Tour-Laendern. 0 = kein Match · 1 = perfekt.</p>
...
with c3:
    ...
    <div class="insight-card">
        ...
        <p>Wie viel % der Streaming-Laender werden auch betourt?
        Misst ob ungenutzte Maerkte existieren.</p>
    </div>""", unsafe_allow_html=True)
...
k5.metric("Median aligned Laender", f"{int(n_aligned_m)}")
st.divider()

# ══════════════════════════════════════════════════════════════════════════
# GA2 — GRAPH 1: Scatterplot Listeners vs Jaccard
# ══════════════════════════════════════════════════════════════════════════
st.markdown('<div class="section-title">📈 Graph 1 — Popularitaet vs. Geo-Alignment (Jaccard)</div>',
            unsafe_allow_html=True)
st.markdown("""
Jeder Punkt = ein Artist. X = Last.fm Listeners (log), Y = Jaccard-Similarity.
**Sind bekanntere Artists besser geo-aligned — oder touren sie in mehr neue Maerkte?**
""")
...
interpretation_g1 = (
    "Positivkorrelation: Populaerere Artists sind besser geo-aligned — sie touren gezielter dorthin wo sie Fans haben."
    if r_g1 > 0.1 and p_g1 < 0.05 else
    "Negativkorrelation: Populaerere Artists touren in mehr Laender als ihr Streaming-Footprint — sie erschliessen aktiv neue Maerkte."
    if r_g1 < -0.1 and p_g1 < 0.05 else
    "Kein signifikanter Zusammenhang — Geo-Alignment ist unabhaengig von der Popularitaet."
)
st.markdown(f"""
<div class="insight-card">
    <h4>💡 Interpretation</h4>
...
Werden populaerere Artists besser oder schlechter geo-aligned?
""")
...
fig_g3.add_trace(go.Bar(
    ...
    name="Streaming-Laender (Last.fm)",
    orientation="h",
    marker_color="#6366f1",
    hovertemplate="%{y}<br>Streaming: %{x} Laender<extra></extra>",
))
fig_g3.add_trace(go.Bar(
    y=top_df["artist_name"],
    x=top_df["n_tour_countries"],
    name="Tour-Laender (Ticketmaster)",
    orientation="h",
    marker_color="#f59e0b",
    hovertemplate="%{y}<br>Tour: %{x} Laender<extra></extra>",
))
fig_g3.add_trace(go.Bar(
    y=top_df["artist_name"],
    x=top_df["n_aligned"],
    ...
    orientation="h",
    marker_color="#10b981",
    hovertemplate="%{y}<br>Aligned: %{x} Laender  Jaccard=%{customdata:.3f}<extra></extra>",
    customdata=top_df["jaccard"],
))
fig_g3.update_layout(
    title=show_type + " — Streaming vs. Tour Laender",
    barmode="group",
    xaxis_title="Anzahl Laender",
    template="plotly_dark",
    paper_bgcolor="#080b14", plot_bgcolor="#161c2d",
...
# ── Zusammenfassung ────────────────────────────────────────────────────────
st.markdown('<div class="section-title">✅ Zusammenfassung — Graphic Analysis 2</div>',
            unsafe_allow_html=True)
...
| **Tour Coverage** | {mean_tc:.1%} | der Tour-Laender sind auch Streaming-Laender |
| **Streaming Reach** | {mean_sr:.1%} | der Streaming-Laender werden betourt |
...
st.markdown(f"""
<div class="insight-card">
    <h4>🎯 Antwort auf Graphic Analysis 2</h4>
    <p>
    ...
    {"Artists touren stark zielgruppenorientiert: wo sie auf Last.fm beliebt sind, spielen sie auch." if mean_jac > 0.4
     else "Eine moderate Ausrichtung: Tour und Streaming-Footprint ueberlappen sich teilweise, aber nicht vollstaendig."
     if mean_jac > 0.2
     else "Streaming-Popularitaet und Tour-Geografie sind weitgehend entkoppelt — viele ungenutzte Maerkte."}
    <br><br>
    Ø Tour Coverage = <strong style="color:#f59e0b">{mean_tc:.1%}</strong>:
    Dieser Anteil der Tour-Laender ist durch Last.fm-Streaming-Popularitaet abgedeckt.
    <br>
    Ø Streaming Reach = <strong style="color:#10b981">{mean_sr:.1%}</strong>:
    Dieser Anteil der Streaming-Laender wird auch tatsaechlich betourt.
    </p>
</div>
""", unsafe_allow_html=True)
...
<div class="methodology-note">
    <p>
    Streaming-Laender = Laender wo der Artist in Last.fm geo.getTopArtists (Top-50) erscheint.
    Tour-Laender = Laender mit mindestens einem Ticketmaster-Event.
    Jaccard-Similarity = |Streaming ∩ Tour| / |Streaming ∪ Tour|.
    Methode robust gegen unterschiedliche Datensatz-Groessen.
